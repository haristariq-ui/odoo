...
class VendorDataWizardLine(models.TransientModel):
        ...
        @api.onchange("select_vendor")
        def onchange_select_vendor(self):
            if self.select_vendor:
                sale_line = self.wizard_id.sale_order_line_id
                if sale_line:
                    sale_line.custom_vendor_id = self.vendor_id